...
| `sync_load` | no | `false` | both | If `true`, offloaded-prefix hits are loaded synchronously: the request is admitted through the same admission path as a plain (cache-miss) request, and the worker blocks in `start_load_kv` until the transfer lands, instead of parking in `WAITING_FOR_REMOTE_KVS` behind an up-front reservation of the whole hit. Recommended for fast local media (CPU/shm over PCIe), where the blocking copy is far cheaper than recomputing the prefix; leave `false` for slow secondary tiers (disk/object/P2P), which would stall the forward pass. Requires all KV cache groups to share one block size. |
