...
| `mamba_keep_every_n_chunks` | no | `1` | both | Mamba/linear-attention (e.g. GDN) groups store their state only for every N-th chunk, plus the prompt's final chunk onward and the shared-prefix junction. A prefix hit needs that state only at its last chunk, so a hit falls back to the previous kept chunk (at most N−1 chunks of recompute). `1` (the default) stores every chunk; e.g. `8` cuts stored bytes about 3x for a 3-GDN + 1-attention model. |
